                                    ...
                                    if ((handshake_type == 1) | (handshake_type == 2))
                                    {
                                        ...
                                        while (exts_len > 0)
                                        {
                                            ...
                                            if (ext_type == 0)
                                            {
                                                ...
                                                // ops[0]->mbuf = server_name;
                                                rte_memcpy(dest_buf, server_name, strlen(server_name) + 1);
                                                if (ops[0]->mbuf)
                                                {

                                                    rte_pktmbuf_attach_extbuf(ops[0]->mbuf,
                                                                              dest_buf, 0, strlen(server_name), &shinfo);

                                                    ops[0]->mbuf->data_len = strlen(server_name);
                                                    ops[0]->mbuf->pkt_len = strlen(server_name);
                                                }
                                                else
                                                {
                                                ...
                                                // const char *temp = rte_pktmbuf_mtod_offset(ops[0]->mbuf, char *, 0);
                                                // printf("The server name is %s and the size is %lu \n", temp, strlen(server_name));
                                                ...
                                                // printf("%u jobs have been enqueued\n", enq_num);
                                                ...
                                                while(nb_enqueue != nb_dequeue){
                                                    nb_dequeue = rte_regexdev_dequeue_burst(0,
                                                                           ...
                                                                           ops,
                                                                           1);}
                                                // printf("%u jobs have been dequeued\n", deq_num);
                                                ...
                                                // printf("The enqueue dequeue process took: %.2f microseconds with %u matches\n", timetaken,nb_matches);
                                                if (nb_matches > 0)
                                                {
                                                    ...
                                                    // printf("A match on the server name %s has been found. Process time: %.2f microseconds \n", server_name, timetaken);
                                                    // printf("match %d, rule = %d, "
                                                    //        "start = %d,len = %d\n",
                                                    //        i, match->rule_id, match->start_offset,
                                                    //        match->len);
                                                    // const char *temp = rte_pktmbuf_mtod_offset(ops[0]->mbuf, char *, 0);
                                                    // for (int i = match->start_offset; i < match->start_offset + match->len; i++)
                                                    // {
                                                    //     printf("%c", temp[i]);
                                                    // }
                                                    // printf("\n");
                                                    ...
                                                    printf("Regex found a match on %s. Process time: %.2f microseconds \n",server_name,timetaken);

                                                    // printf("The server name is %s and the number of matches is %u \n", server_name, nb_matches);
                                                }
                                                ...
                                                //     nb_rx--;
                                                //     rte_pktmbuf_free(bufs[i]);
                                                //     printf("Exact match: %s is on the blacklist with size %lu. Time: %.2f microseconds\n", server_name, strlen(server_name), timetaken);
                                                // }
                                                break;
                                            }
