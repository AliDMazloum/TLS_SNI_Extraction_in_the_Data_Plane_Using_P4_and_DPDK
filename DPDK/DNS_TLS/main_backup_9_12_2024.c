            ...
            if (nb_rx > 0)
            {
                ...
                for (int i = 0; i < nb_rx; i++)
                {
                    ...
                    if(true)
                    {
                        ...
                        pIP4Hdr = rte_pktmbuf_mtod_offset(bufs[i], struct rte_ipv4_hdr *,0);
                        uint8_t IPv4NextProtocol = pIP4Hdr->next_proto_id;
                        ...
                        if (IPv4NextProtocol == 6)
                        {
                            ...
                            if (dst_port == 443)
                            {
                                ...
                                // int total_length = tlsdata_offset;
                                pTlsHdr = rte_pktmbuf_mtod_offset(bufs[i], struct rte_tls_hdr *, tcpdata_offset);
                                uint8_t tls_type = pTlsHdr->type;
                                uint32_t tlsdata_offset = tcpdata_offset + sizeof(struct rte_tls_hdr);
                                if (tls_type == 0x16)
                                    ...
                                    if ((handshake_type == 1))
                                    {
                                        ...
                                        while (exts_len > 0)
                                        {
                                            ...
                                            if (ext_type == 0)
                                            {
                                                ...
                                                // struct rte_mempool *mbuf_pool = rte_pktmbuf_mtod_offset(bufs[i], struct rte_mempool *, tlsdata_offset);
                                                ...
                                                // printf("The total length is %i \n",rte_cpu_to_be_16(pIP4Hdr->total_length)-total_length);
                                                ...
                                                //                               pTlsHandshakeHdr, 0, rte_cpu_to_be_16(pIP4Hdr->total_length)-total_length, &shinfo);

                                                //     ops[0]->mbuf->data_len = rte_cpu_to_be_16(pIP4Hdr->total_length)-total_length;
                                                //     ops[0]->mbuf->pkt_len = rte_cpu_to_be_16(pIP4Hdr->total_length)-total_length;
                                                // }
                                                if (ops[0]->mbuf)
                                                {
                                                ...
                                                //     rte_pktmbuf_attach_extbuf(ops[0]->mbuf,
                                                //                               ethernet_header, 0, 8000, &shinfo);

                                                //     ops[0]->mbuf->data_len = 8000;
                                                //     ops[0]->mbuf->pkt_len = 8000;
                                                // }
                                                else
                                                {
                                                ...
                                                if (nb_matches > 0)
                                                {
                                                    ...
                                                    // printf("Regex found a match on %s\n",server_name);
                                                }
